...
| `registry_match` | INTEGER DEFAULT 0 | 1 = moneda verificada contra catálogo oficial (habilita claim onchain). Dos fuentes de actualización: (1) el loader de `/mycollection` lo recalcula en memoria al listar la colección; (2) el action `fix_registry_match` del panel `/admin` hace un batch UPDATE persistente en D1 para todas las monedas. `api.rewards.request` verifica en memoria (no lee este campo de la DB para autorizar claims). |

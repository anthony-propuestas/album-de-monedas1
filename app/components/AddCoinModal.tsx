...
export function AddCoinModal({ isOpen, onClose }: Props) {
  ...
  const handleFile = useCallback((slot: string, file: File | null) => {
    if (file) {
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input name="mint" placeholder="Ej: Casa de Moneda de México" className={INPUT} />
            </div>
            <div className="flex flex-col gap-1.5">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              {PHOTO_SLOTS.map((slot) => (
                <label
                  key={slot.key}
                  className="relative flex flex-col items-center justify-center aspect-square rounded-xl border-2 border-dashed border-[rgba(210,180,130,0.2)] hover:border-[rgba(210,180,130,0.45)] cursor-pointer overflow-hidden transition-colors bg-[rgba(14,11,10,0.5)]"
                >
                  <input
                    type="file"
                    name={slot.key}
